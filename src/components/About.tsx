...
import { AboutData, PortfolioData } from '../types';
...
const About: React.FC<AboutProps> = ({ data }) => {
    const [aboutItems, setAboutItems] = useState<AboutData | null>(null);
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:max-w-6xl lg:px-8">
                <div className="w-11/12 md:w-full mx-auto text-center mb-4 md:mb-8">
                    ...
                    <p className='text-gray-300 font-medium text-center md:text-justify text-lg md:text-xl mb-4'>
                        {aboutItems?.section_body?.quotes}</p>
                    <p className='text-gray-300 text-xs italic text-center md:text-justify text-lg md:text-sm mb-4'>
                        {aboutItems?.section_body?.quotes_alt}</p>

                </div>
                <div className="w-w-1/3 md:w-full px-4 mb-2 md:mb-8">
                    <img src={aboutItems?.section_body?.profile_image} className="w-full h-auto shadow-md [clip-path:polygon(22.2%_0%,100%_0%,100%_100%,0%_100%,_0_18.3%)]" alt={aboutItems?.section_body?.profile_alt} />
                </div>
            </div>
        </section>
    );
